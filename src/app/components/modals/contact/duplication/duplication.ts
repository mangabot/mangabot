...
@Component({
    selector: 'cm-duplication',
    templateUrl: 'app/components/modals/contact/duplication/duplication.html',
    styleUrls: ['app/components/modals/contact/duplication/duplication.css']
})
export class CmDuplication {
    private id = "#cm-duplication";
    ...
    constructor(private contactService: ContactService, actionStream: ActionStream) {
        actionStream.getStream()
            .filter(action => action.id == 'show-contact-duplication')
            .subscribe(action => {
    ...
    show(contact: Contact) {
        this.loading = true;
        this.contactMap = {};
        this.duplication.cases = contact.getDuplicateCases();
        this.duplication.total = this.duplication.cases.length;
        jQuery(this.id).modal('show');
        let ids = [contact.id].concat(contact.duplicateIds);
